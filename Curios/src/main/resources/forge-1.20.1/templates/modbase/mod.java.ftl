...
@Mod("${modid}") public class ${JavaModName} {
	...
	public ${JavaModName}() {
		...
		<#if w.getGElementsOfType("dyeable_item")?filter(e -> e.isDyeable)?size != 0>
		...
		<#if w.getGElementsOfType("dyeable_item")?filter(e -> e.isDyeable)?size != 0>${JavaModName}RecipeSerializers.REGISTRY.register(bus);</#if>
